    ...
    axios
      ...
      .then(function (response) {
        ...
        $("li.stream-item").each(function (index) {
          ...
          var date =
            $(this).find(".tweet-timestamp").find("span").data("time") * 1000; // Convert seconds to milliseconds
      ...
      .then((final) => {
        ...
        final.forEach((e) => {
          News.findOne({ date: e.date }).then((news) => {
            if (news) {
              ...
              if (e.profile == "ForexLive" || e.profile == "CNBC")
                e.content = e.content.split("http", 1);
              e.save();
            }
          });
        });
